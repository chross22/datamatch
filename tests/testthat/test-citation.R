...
#   * no inst/CITATION, so citation("datamatch") falls back to an auto-generated
...
test_that("inst/CITATION exists and parses", {
  ...
  parsed <- utils::readCitationFile(
    cit, meta = list(Package = "datamatch", Version = ver)
  )
...
test_that("the README says how to cite the package", {
  ...
  expect_match(txt, "datamatch", fixed = TRUE)
})
