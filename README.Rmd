...
Seven sources sit behind one interface, sharing one set of variable names:
...
> Copernicus, FVCOM, HYCOM and CEFI are four different models, and `CHL` from
> Copernicus, ERDDAP, OB.DAAC and CEFI four different things — three retrievals
> and a simulation. Sharing the column name is what
...
`cefi_dictionary()` and `obdaac_dictionary()` do the same for the other six
sources, and `as_markdown()` renders any of them as a pipe
table.
...
regional model — MOM6-COBALT run for the **Northwest Atlantic at a twelfth of a
degree**, from the US east coast to the Caribbean, 1993–2023. No account
needed.

It is the only source here that carries **coupled physics and biogeochemistry
on one grid at one resolution**: temperature and salinity beside nitrate,
oxygen, pH, pCO2, phytoplankton carbon and mesozooplankton biomass. The
Copernicus biogeochemical reanalysis is a quarter degree; this resolves the Gulf
of Maine rather than smoothing across it. It also publishes **sea-floor salinity
outright** as `sob`, so `BOTS` needs none of the deepest-wet-level derivation
GLORYS forces.

Two things to know. **It is one region** — outside roughly 98°W–36°W and
5°N–58°N there is nothing, and a box outside the domain is refused rather than
returning an empty join. And **the daily output is biogeochemistry only**:
`CHL`, `NO3`, `PH`, `PCO2`, `PHYC`, `MESOZOO` and `BOTO2` have a daily step,
everything else is monthly, and a daily request for `SST` says so.

`cefi_variables()` and `cefi_dictionary()` list the catalog; `cefi_archives()`
names the runs that ship, and `cefi_archive()` reaches the domains that do not —
Northeast Pacific, Arctic, Pacific Islands, Great Lakes — the same way
`fvcom_archive()` does for FVCOM.

> **Forecasts are experimental.** `experiment = "decadal_forecast"` reads a
> prediction rather than a reconstruction, warns on every call, and will not
> choose for you: `init` says which of the sixty initialisations to read and
> `member` which of the ten ensemble members, because averaging members is a
> modelling decision this package will not make silently. CEFI's seasonal
> forecast cannot be read here at all — its files carry a 64-bit coordinate that
> OPeNDAP cannot serve — and `accessCEFI()` says so with the workaround.
...
`accessOBDAAC()` reads Level-3 mapped ocean colour from NASA's Ocean Biology
DAAC. What it adds over `accessERDDAP()` is **length**: SeaWiFS from September
1997, then both MODIS instruments and all three VIIRS, all reprocessed together,
where the ERDDAP entries start in 2012 at the earliest.
...
Three things to know. **It is the one source needing a NASA Earthdata Login**,
and the failure without one is misleading: NASA answers an unauthenticated
request with HTTP 200 and the login page, so `accessOBDAAC()` checks the bytes
and refuses it by name rather than writing HTML into a `.nc` file. Register once
and generate an appkey, then put it in `~/.Renviron` as `EARTHDATA_APPKEY=`.
**There is no server-side subsetting** — a variable-day is one global file, about
15 MB at 4 km and 5 MB at 9 km, subset locally and cached. And **daily fields are
gappy**: these are single-sensor composites, so outside the tropics a daily `CHL`
is mostly cloud. Use `frequency = "monthly"`, or `fill_satellite_gaps()`.

> **The sensors are not interchangeable.** Where they overlap they disagree, and
> stitching a series across a mission boundary puts an instrumental step in it.
> `source_of()` records which one answered. Where a long consistent record
> matters more than any one sensor, the Copernicus-GlobColour entries are
...
`hycom_variables()`, `hycom_archives()`, `cefi_variables()`, `cefi_archives()`,
`ccmp_variables()`, `ccmp_versions()`, `erddap_datasets()`,
`obdaac_variables()` and `obdaac_sensors()` for the rest. `accessEnvDat()` is the old name for
...
`cefi_archives()`, `ccmp_versions()` and `obdaac_sensors()` carry their
