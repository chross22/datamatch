...
- **`accessFVCOM()`** — FVCOM, an unstructured-mesh coastal model.
  ...
  themselves, for plotting the grid.
- **`accessHYCOM()`** — HYCOM GOFS 3.1, an independent global model that
  publishes sea-floor temperature and salinity as fields. Three-hourly,
  1994–2024 across archives; `hycom_covering(date)` says which one holds
  a day.
- **`accessCCMP()`** — CCMP surface winds, six-hourly from 1993 to the
  present. The longest wind record here, but no stress.
- **`accessERDDAP()`** — MUR sea surface temperature at 0.01°, and VIIRS
  chlorophyll, from NOAA’s ERDDAP servers. `erddap_dataset()` reaches
  any of the thousands of other datasets they host.
...
- [The access functions](#the-access-functions) — what each of the five
  gives
...
### Plotting the mesh itself

`accessFVCOM()` returns values at points — nodes or element centroids —
which is what matching needs but is not the grid. `fvcom_mesh()` returns
the triangles, so the mesh can actually be drawn:
...
`plot_mesh()` does the spatial join for you, which is worth having: a
fetch is subset to your box, so its row order says nothing about the
mesh’s own element numbering, and joining by position would silently
mislabel every cell. Node values sit at triangle *corners* and element
values at centroids, and it handles both. Reading it needs the `ncdf4`
...
| `plot_mesh()`     | an FVCOM mesh, bare or shaded by a covariate     |
...
| `fvcom_variables()`, `fvcom_dictionary()`, `fvcom_archives()`, `fvcom_archive()`, `fvcom_mesh()` | FVCOM |
