...
| [FVCOM / NECOFS](#fvcom-a-regional-model-on-an-unstructured-mesh) | `accessFVCOM()` | coastal model on a triangular mesh | monthly | 1978–2013 |
| [HYCOM](#hycom-and-bottom-fields-for-free) | `accessHYCOM()` | independent global model, sea-floor fields | 3-hourly | 1994–2015 |
...
- [Set up](#set-up) — the Copernicus client, sign-in, and where
...
- [Quick start](#quick-start)
  ...
  - [Downloads run in parallel](#downloads-run-in-parallel) —
    `n_workers`, and what is already cached
...
- [Resampling](#resampling) — moving between grids and time steps
  - [Aggregating loses detail, which is the safe
    direction](#aggregating-loses-detail-which-is-the-safe-direction)
  - [Interpolating adds cells, not
    information](#interpolating-adds-cells-not-information)
- [Filling satellite gaps](#filling-satellite-gaps) — substituting the
  model where cloud blocked the view
...
- [Static and basin-scale
  ...
  - [Climate indices](#climate-indices) — NAO, AO, AMO, PDO, LCR, AMOC
    ...
    - [The Labrador Current retroflection
      index](#the-labrador-current-retroflection-index)

**Putting it to use**

- [Putting it together](#putting-it-together) — a full worked example,
  four sources onto one table
- [Matching](#matching) — a general spatiotemporal join, not just
  observations
- [Looking things up](#looking-things-up) — dictionaries, catalogs, and
  small helpers
...
across calls — each `accessEnvDat()` call is one dataset, so fetching
SST and CHL is two calls, run one after the other with each parallel
inside itself.
...
it useful on gappy satellite data, where `fill_satellite_gaps()` is not
an option.

Resampled output keeps the `YEAR`/`MONTH`/`DAY` stamping convention, so
`matchData()` reads its resolution back correctly and can be used on the
result directly.
...
`plot_env()` maps one variable for one time step. It is the first thing
worth doing after a download. It is also the fastest way to catch a
bounding box that landed somewhere unintended, a variable that is
entirely `NA`, or a depth range that returned the wrong level.
...
`plot_coverage()` is the one to run before trusting a satellite series.
It plots the fraction of cells carrying a value in each time step. On
ocean colour the shape is stark: near-complete in summer, a quarter of
the grid in winter.

That picture decides three things. Whether a monthly mean is worth
having. Whether `fill_satellite_gaps()` is worth the seam it introduces.
And where to set `min_coverage` when aggregating.
...
`plot_series()` reduces each time step to one number over the study
area. That is how a seasonal cycle, a trend, or a step change at a
...
`plot_matched()` shows what the join actually produced. Observations
that matched nothing are drawn as open circles rather than dropped. A
cluster of them is usually the real finding: observations outside the
environmental data’s extent, or in a period it does not cover.
...
`accessHYCOM()` reads the HYCOM + NCODA GOFS 3.1 reanalysis (`GLBv0.08`
`expt_53.X`, 1994–2015) from the Naval Research Laboratory’s THREDDS
server:
...
`bathymetry_variables()` lists them: `DEPTH`, `SLOPE`, `ASPECT`, and
`TPI`.
...
`attach_bathymetry()` takes either a plain data frame with coordinate
columns or an `sf` object, so it works on observations and on
`accessEnvDat()` output alike.
...
with one fitted to `NAO`. `index_dictionary()` carries the units at
runtime.
...
One interaction to know. `attach_climate_index()` joins on year and
month, and `upscale_time(to = "year")` stamps its output `MONTH = 1`. So
attaching an index to annual data would give every year January’s value.
Aggregate the index to a year first instead.
...
Small helpers for asking the package what it knows, rather than reading
the source or guessing:
...
The `_dictionary()` pair print as tables and are what to read; the plain
catalog functions return lists and are what to write code against.
`as_markdown()` renders either dictionary for pasting into
documentation.
